...
class MA_cross_Sentiment(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.ema200_sentiment = bt.indicators.ExponentialMovingAverage(
            self.data.lines.sentiment, period=2000, subplot=True
        )
        self.ema50_sentiment = bt.indicators.ExponentialMovingAverage(
            self.data.lines.sentiment, period=500, plotmaster=self.ema200_sentiment
        )

        self.orefs = list()
        self.order = None

        self.stoploss_percentage = 0.2
        self.takeprofit_percentage = 0.3
        self.stake = 100  # Kolik nakupuju nebo prodávám
